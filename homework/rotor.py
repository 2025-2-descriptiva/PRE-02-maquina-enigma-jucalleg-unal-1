 ...
 un mapeo interno fijo entre las 26 letras del alfabeto. En la Enigma militar
   existían cinco rotores. Cada rotor tenía un cableado interno distinto y uno o
     más puntos de muesca que determinaban cuándo debía avanzar el siguiente rotor,
       generando el efecto de "odómetro". El mapeo no podia ser cambiado por el usuario.

El operador podia seleccionar el orden en el que se montaban los rotores, 
definir su posición inicialy ajustar la configuración del anillo interno (Ringstellung).
 Estas variaciones, combinadas con los demás componentes de la máquina, incrementaban de forma exponencial el espacio 
 de claves posibles.

A diferencia del plugboard y del reflector, cada rotor gira una posición con cada pulsación de tecla,
alterando dinámicamente el recorrido de la señal y produciendo asi una sustitución distinta en cada paso.
